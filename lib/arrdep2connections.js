...
function ArrDep2Connections (arrivalsStream, options) {
  ...
  if (this._mongoDb) {
    ...
    MongoClient.connect(url, function(err, db) {
      ...
      this._collection = db.collection(options.mongoDbConfig.collection);
      this._collection.remove(); // clean up previous collection
    });
  }
}
...
ArrDep2Connections.prototype._processArrival = function (departure, arrival, next, done) {
  ...
  if (arrivalDateTime <= departureDateTime) {
    ...
    if (this._mongoDb) {
      this._addConnectionToMongoDB(connection);
    }
...
ArrDep2Connections.prototype._addConnectionToMongoDB = function (connection) {
  _collection.insert(connection, function(err, result) {
    if (!err) {
      // console.error("Inserted connection into the connection collection of MongoDB");
    } else {
      die (err);
    }
  });
